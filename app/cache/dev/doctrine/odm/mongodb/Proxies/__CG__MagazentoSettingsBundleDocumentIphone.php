<?php

namespace MongoDBODMProxies\__CG__\Magazento\SettingsBundle\Document;

use Doctrine\ODM\MongoDB\Persisters\DocumentPersister;

/**
 * THIS CLASS WAS GENERATED BY THE DOCTRINE ODM. DO NOT EDIT THIS FILE.
 */
class Iphone extends \Magazento\SettingsBundle\Document\Iphone implements \Doctrine\ODM\MongoDB\Proxy\Proxy
{
    private $__documentPersister__;
    public $__identifier__;
    public $__isInitialized__ = false;
    public function __construct(DocumentPersister $documentPersister, $identifier)
    {
        $this->__documentPersister__ = $documentPersister;
        $this->__identifier__ = $identifier;
    }
    /** @private */
    public function __load()
    {
        if (!$this->__isInitialized__ && $this->__documentPersister__) {
            $this->__isInitialized__ = true;

            if (method_exists($this, "__wakeup")) {
                // call this after __isInitialized__to avoid infinite recursion
                // but before loading to emulate what ClassMetadata::newInstance()
                // provides.
                $this->__wakeup();
            }

            if ($this->__documentPersister__->load($this->__identifier__, $this) === null) {
                throw \Doctrine\ODM\MongoDB\DocumentNotFoundException::documentNotFound(get_class($this), $this->__identifier__);
            }
            unset($this->__documentPersister__, $this->__identifier__);
        }
    }

    /** @private */
    public function __isInitialized()
    {
        return $this->__isInitialized__;
    }

    
    public function getId()
    {
        if ($this->__isInitialized__ === false) {
            return $this->__identifier__;
        }
        $this->__load();
        return parent::getId();
    }

    public function setUserId($userId)
    {
        $this->__load();
        return parent::setUserId($userId);
    }

    public function getUserId()
    {
        $this->__load();
        return parent::getUserId();
    }

    public function setProductName($productName)
    {
        $this->__load();
        return parent::setProductName($productName);
    }

    public function getProductName()
    {
        $this->__load();
        return parent::getProductName();
    }

    public function setIcon($icon)
    {
        $this->__load();
        return parent::setIcon($icon);
    }

    public function getIcon()
    {
        $this->__load();
        return parent::getIcon();
    }

    public function setIconRetina($iconRetina)
    {
        $this->__load();
        return parent::setIconRetina($iconRetina);
    }

    public function getIconRetina()
    {
        $this->__load();
        return parent::getIconRetina();
    }

    public function setDefault($default)
    {
        $this->__load();
        return parent::setDefault($default);
    }

    public function getDefault()
    {
        $this->__load();
        return parent::getDefault();
    }

    public function setDefaultRetina($defaultRetina)
    {
        $this->__load();
        return parent::setDefaultRetina($defaultRetina);
    }

    public function getDefaultRetina()
    {
        $this->__load();
        return parent::getDefaultRetina();
    }

    public function setDefaultRetina568h($defaultRetina568h)
    {
        $this->__load();
        return parent::setDefaultRetina568h($defaultRetina568h);
    }

    public function getDefaultRetina568h()
    {
        $this->__load();
        return parent::getDefaultRetina568h();
    }


    public function __sleep()
    {
        return array('__isInitialized__', 'id', 'user_id', 'product_name', 'icon', 'icon_retina', 'default', 'default_retina', 'default_retina_568h');
    }

    public function __clone()
    {
        if (!$this->__isInitialized__ && $this->__documentPersister__) {
            $this->__isInitialized__ = true;
            $class = $this->__documentPersister__->getClassMetadata();
            $original = $this->__documentPersister__->load($this->__identifier__);
            if ($original === null) {
                throw \Doctrine\ODM\MongoDB\MongoDBException::documentNotFound(get_class($this), $this->__identifier__);
            }
            foreach ($class->reflFields AS $field => $reflProperty) {
                $reflProperty->setValue($this, $reflProperty->getValue($original));
            }
            unset($this->__documentPersister__, $this->__identifier__);
        }
        
    }
}